import psycopg2
import psycopg2.extras
import sys
import cgi
import cgitb
import os
import json

cgitb.enable()
#print "Content-type:text/html\r\n\r\n"
print 'Content-Type: text/plain\r\n\r'

try:
    conn = psycopg2.connect("dbname='info1003_project' user='info1003user' host='postgres.it.usyd.edu.au' password='userpj1003'")
    # Parsing input data
    urlString = "?" + os.environ['QUERY_STRING']
    #urlString = "?username=pikachu&numfeeds=10"
    from urlparse import urlparse, parse_qs
    qs = urlparse(urlString).query
    dataJson = parse_qs(qs)
    userName = dataJson.get('username')[0]
    userName = "'" + userName + "'"
    numFeeds = dataJson.get('numfeeds')[0]
    cursor = conn.cursor()
    #print userName, numFeeds

    # execute Query
    cursor.execute("SELECT * FROM feed WHERE ownerId_fk = " + userName + " ORDER BY createdat DESC limit " + numFeeds)
    # retrieve the records from the database
    records = cursor.fetchall()
    colName = ['feedid_pk', 'userfrom_fk', 'ownerid_fk', 'activitytype', 'verb', 'action', 'createdat']
    
    out = []
    for row in range(len(records)):
        out.append({})
        for j in range(len(records[row])):
            out[row][colName[j]] = records[row][j]

    jsonData = json.dumps(out)
    print jsonData
    
except:
    # Get the most recent exception
    exceptionType, exceptionValue, exceptionTraceback = sys.exc_info()
    # Exit the script and print an error telling what happened.
    sys.exit("Database connection failed!\n ->%s" % (exceptionValue))
