...
try:
    ...
    friendName = "'" + dataJson.get('friendname')[0] + "'"
    cursor = conn.cursor()
    
    # DELETING FRIENDSHIP
    cursor.execute("DELETE FROM friendship WHERE userid_fk = " + userName + " AND frienduserid = " + friendName)
    cursor.execute("DELETE FROM friendship WHERE userid_fk = " + friendName + " AND frienduserid = " + userName)      
    conn.commit()
    print "success"
except:
